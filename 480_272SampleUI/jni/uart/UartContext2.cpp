...
#include "uart/UartContext2.h"
#include "utils/Log.h"

#define UART_DATA_BUF_LEN			16384	// 16KB

UartContext2::UartContext2() :
	mIsOpen(false),
	mUartID(0),
	mDataBufPtr(NULL),
	mDataBufLen(0) {
...
bool UartContext2::openUart(const char *pFileName, UINT baudRate) {
...
bool UartContext2::send(const BYTE *pData, UINT len) {
	if (!mIsOpen) {
		return false;
	}
...
UartContext2* UartContext2::getInstance() {
	static UartContext2 sUC;
	return &sUC;
}

bool UartContext2::readyToRun() {
	if (mDataBufPtr == NULL) {
...
bool UartContext2::threadLoop() {
	if (mIsOpen) {
		...
		if (readNum > 0) {
			...
			// 解析协议
			int len = mDataBufLen;//parseProtocol(mDataBufPtr, mDataBufLen);
