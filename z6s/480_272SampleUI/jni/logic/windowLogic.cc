...
static bool onUI_Timer(int id){
	...
	if(brefresh){
		brefresh = 0;
		mWindownormalPtr->setPosition(ps);
	}
	return true;
}


static bool isInArea(const MotionEvent &ev,LayoutPosition &pos){
	if(ev.mX < pos.mLeft) return false;
	if(ev.mY < pos.mTop) return false;
	if(ev.mX > pos.mLeft+pos.mWidth) return false;
	if(ev.mY > pos.mTop+pos.mHeight) return false;
...
static bool onwindowActivityTouchEvent(const MotionEvent &ev) {
	...
	ps = mWindownormalPtr->getPosition();
